...
def draft(report, review, root):
    """Explicit selection creates a NEW intent; never overwrites an existing one.

    The root is mandatory: analysis of yesterday's pixels cannot silently attach
    today's replacement. Ordinary Studio staging/preflight must check again.
    """
    ...
    need(root is not None, 'A source workspace is required to recheck reference bytes')
    ...
    for ref, image, choice in zip(q['references'], answer['images'], choices):
        ...
        file_bytes(root, ref, 8 * 1024 * 1024)
        takes = []
