...
2026): the requested re-test with the pack's own portrait as the style board already ran on 23 September 2026 — 18 Studio
jobs in `experiments/curated/overnight-20260923/stylepose-pack/README.md` (look-B portrait as the board, standing and
action poses, seeds 2026092371–73, pose strengths 0.7/0.9/1.0, default style weight 0.7), judged open by the lab with the
independent second judge blind in `docs/quality/second-judge/stylepose-pack.md`. No duplicate run is needed.
...
- [x] **prepare-large-job-proof (#306)**: The owner approved the full Windows proof, including the backend restart, and it ran on 25 September 2026. The [retained receipt](docs/verification/2026-09-25-large-job-preparation-live-proof.md) records the refusal cases, before/after counters, release, history check and owned restart. The proof found three defects (#972, #980, #982), recovered 2.75 GB of commit headroom, and did not terminate or restart the owner's LLM process. The original config was restored byte-for-byte; the command remains off by default because its two flags are absent from `config/local.json`. This completes the human action recorded here; it does not itself close issue #306.
