...
4. Prove: `python scripts/validate-repo.py`, then `python -m unittest tests.test_server`. If the change
   touched binding logic, add a test next to the existing `FakeStudio` cases.
5. With ComfyUI running, `python scripts/validate-live.py` checks node classes without queueing work.
   Restart the Studio server so the catalog reloads.
6. Hand off to `studio-execution-evidence` for the single proving generation, then flip `verified`
...
`.claude/rules/catalog.md` region rule (auto-loads on these paths).
