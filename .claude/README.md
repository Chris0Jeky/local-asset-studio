...
| Path | Purpose |
| --- | --- |
| `settings.json` | attribution trailers off; allow rules for the proving checks, read-only Git, `gh` per subcommand, `rg`. No `defaultMode` (a project-scope mode would outrank the user's own), no hooks, no deny list: the repo is declared floorless in `../.agent-harness/tier.json`. Rules are prefix rules (`:*` only wildcards at the end of the token). |
