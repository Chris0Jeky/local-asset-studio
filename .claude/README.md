...
Repo facts and proving checks live in `../CLAUDE.md`; the Codex adapter in `../AGENTS.md`; the Grok
adapter in `../.grok/`; review and tier doctrine in the global laws. Nothing is restated here.
...
| `skills/` | canonical repo-local skills (`studio-*`); `../.codex/skills/` is the body-identical Codex adapter; Grok loads this tree via Claude compatibility. `tests/test_agent_harness.py` fails on Codex drift and on a third `.grok/skills/` copy. |
