...
| FLUX.2 Klein 4B fp8 | yes (`flux-2-klein-4b-fp8`) | 16–44 s per edit (character-reference pilot, 12 Sep) | native reference edit (`flux-edit`), up to 4 references; AniEdit LoRA adds anime style transfer from references | Apache-2.0 (base); AniEdit LoRA is FLUX.1-dev non-commercial | **Best candidate for "accurate modification"** (instruction edits, keep identity). The pilot rejected 10/11 for identity fidelity, so it needs its own proving pass with AniEdit before it can carry the Restyle route. |
| FLUX.2 Klein 9B | no | — | same as 4B, stronger | **non-commercial**; 24 GB+ recommended, 16 GB only at reduced resolution | Not for this card and not for a pack the owner may sell. Skip. |
