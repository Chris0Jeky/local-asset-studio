...
text = tpl.replace(ph[0], "Ellen Joe in pink shorts and top with attractive silhouette").replace(ph[1], "leaning forwards, seen from the back, one hand on the hip moving the shorts slightly") + " Leave out the tail."
...
g["13"]["inputs"]["filename_prefix"] = "Research/combine-fix"
json.dump({"text": text}, open(OUT + "/prove_fix.wording.json", "w", encoding="utf-8"), indent=1)
results = []
for seed in (2026091411, 2026091412, 2026091413, 2026091414):
...
json.dump(results, open(OUT + "/prove_fix.results.json", "w"), indent=1)
...
sheet.save(OUT + "/combine-fix-sheet.jpg", quality=85); print("sheet", sheet.size)
