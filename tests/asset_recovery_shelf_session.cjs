...
function setup({shared=device(),tabStorage=null}={}){const journal=Journal.create(storage()),store=Core.create({storage:shared.local,locks:shared.locks,crypto});assert.equal(typeof Session.create,'function','shelf session controller must exist');const messages=[],session=Session.create({journal,store,crypto,tabStorage,onStatus:s=>messages.push(s)});return {journal,store,session,messages};}
...
test('identical snapshots in separate tabs never share shelf ownership',async()=>{
  const shared=device(),a=setup({shared,tabStorage:storage()}),b=setup({shared,tabStorage:storage()});
