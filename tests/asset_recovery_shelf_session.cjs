...
function setup(){const journal=Journal.create(storage()),local=storage();let tail=Promise.resolve();const locks={request(n,o,fn){const p=tail.then(fn);tail=p.catch(()=>{});return p;}};const store=Core.create({storage:local,locks,crypto});assert.equal(typeof Session.create,'function','shelf session controller must exist');const messages=[],session=Session.create({journal,store,crypto,onStatus:s=>messages.push(s)});return {journal,store,session,messages};}
