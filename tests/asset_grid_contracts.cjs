...
test('A run label change repaints the card (#939)',()=>{assert.notEqual(grid.displayKey(asset,false),grid.displayKey({...asset,run_label:'Agent lab'},false));assert.equal(grid.displayKey(asset,false),grid.displayKey({...asset,run_label:null},false));});
test('Group review and source controls have distinct focus keys',()=>{assert.equal(grid.actionKey({dataset:{groupReview:'rejected'}}),'rejected');assert.equal(grid.actionKey({dataset:{groupSource:'agent'}}),'source:agent');assert.equal(grid.actionKey({dataset:{}}),null);assert.equal(grid.actionKey(null),null);});
