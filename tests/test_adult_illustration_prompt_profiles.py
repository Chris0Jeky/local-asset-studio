...
def sample_projection(reference_count: int = 3) -> dict:
    ...
    return {
        "format": "studio.adult-illustration.projection/v1",
        "intent": {
            "format": "studio.adult-illustration.intent/v1",
            "id": "hot-spring-study",
            "content": {
        ...
        "source_sha256": "1" * 64,
        "state": "requires_binding",
        "creative_intent": {
            "schema_version": 1,
            "id": "hot-spring-study",
            "task": "image",
            "brief": (
                "Create a controlled, sensual but non-explicit anime fashion illustration "
                "of an approved adult original character on a private hot-spring veranda."
            ),
            "facets": {
                "subject": "approved adult original character; short black hair; amber eyes",
                "action": "sitting sideways with a torso twist and natural weight support",
                "setting": "private hot spring veranda at night",
                "camera": "three-quarter full-body framing from slightly below eye level",
                "lighting": "warm rim light and cool moonlight",
                "style": "clean anime linework; soft silk and steam",
            },
            "tags": ["solo", "sitting", "torso_twist", "looking_at_viewer", "steam", "mystery_token"],
            "avoid": ["text", "watermark", "copied background"],
            "constraints": [
                {
                    "id": "adult-status",
                ...
                {
                    "id": "coverage",
                    "text": "Preserve opaque critical coverage.",
                    "mechanism": "verify",
                    "priority": "hard",
                },
            ],
            ...
            "verbatim": {},
            "parameters": {},
            "locked": ["constraints", "references", "verbatim"],
        },
        "control_plan": [
            {
                "id": "pose-guide",
                "mechanism": "geometry_artifact",
                "description": "Bind a reviewed full-body pose artifact.",
                "status": "requires_route_binding",
            },
            {
                "id": "coverage-review",
                "mechanism": "review_decision",
                "description": "Retain coverage in the reviewed result.",
...
class PromptProfileCatalogTests(unittest.TestCase):
    ...
    def test_catalog_rejects_alias_collision(self) -> None:
        ...
        data["entries"][1]["aliases"].append(data["entries"][0]["canonical"].replace(" ", "_"))
...
class PromptProjectionTests(unittest.TestCase):
    def test_animagine_compiles_verified_ordered_tags_and_keeps_unknowns_visible(self) -> None:
        ...
        self.assertTrue(positive.endswith("masterpiece, high score, great score, absurdres"))
        ...
        self.assertIn("UNKNOWN_VOCABULARY", {item["code"] for item in result["diagnostics"]})
        self.assertIn("CONTROL_REQUIRES_ROUTE_BINDING", {item["code"] for item in result["diagnostics"]})
    ...
    def test_qwen_compiles_ordered_reference_ownership_without_tag_dump(self) -> None:
        ...
        self.assertIn("Do not copy: source outfit, source pose, source background", instruction)
        ...
        self.assertEqual([item["id"] for item in result["reference_bindings"]], [
    ...
    def test_qwen_refuses_more_references_than_the_pinned_graph_profile(self) -> None:
        with self.assertRaisesRegex(ValueError, "at most 3 references"):
            compile_prompt(sample_projection(reference_count=4), "qwen-edit-2511-instruction-v1", ROOT)
    ...
    def test_compilation_is_deterministic_and_validation_detects_tampering(self) -> None:
        ...
        second = compile_prompt(copy.deepcopy(projection), "animagine-xl4-ordered-v1", ROOT)
    ...
    def test_invalid_or_authoritative_source_projection_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "must not authorize execution"):
