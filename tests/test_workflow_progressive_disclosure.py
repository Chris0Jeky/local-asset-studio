...
class WorkflowProgressiveDisclosureTests(unittest.TestCase):
    ...
    def test_agent_manual_is_collapsed_discoverable_and_the_tab_opens_it(self):
        ...
        self.assertIn('wf-visually-hidden', title['attrs'].get('class', '').split())
        self.assertIn('agents', title['ancestors'])
        self.assertNotIn('agentGuide', title['ancestors'], 'The section heading must not rely on summary descendants')
        self.assertNotIn('role="heading"', self.source)
        self.assertIn('.wf-visually-hidden', self.style)
