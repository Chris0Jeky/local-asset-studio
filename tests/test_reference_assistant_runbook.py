...
class ReferenceAssistantRunbookTests(unittest.TestCase):
    ...
    def test_examples_do_not_embed_one_machine_absolute_path(self):
        ...
        self.assertNotIn("$Workspace = Join-Path (Get-Location) 'reference-session'", self.text)
        self.assertNotIn("workspace = Path('reference-session')", self.text)
        self.assertIn("$Workspace = Join-Path (Get-Location) '.runtime/reference-session'", self.text)
        self.assertIn("workspace = Path('.runtime/reference-session')", self.text)
