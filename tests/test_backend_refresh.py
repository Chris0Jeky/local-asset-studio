...
class BackendRefreshContracts(unittest.TestCase):
    ...
    def test_current_editor_and_observation_ownership(self):
        result = subprocess.run([shutil.which('node'), '--test', '--test-reporter=tap', str(Path(__file__).with_name('backend_refresh.cjs'))], capture_output=True, text=True, timeout=20)
