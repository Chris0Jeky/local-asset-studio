...
const nodes=new Map();
function node(selector){if(!nodes.has(selector))nodes.set(selector,{innerHTML:'',textContent:'',hidden:false,value:'',checked:false,classList:{toggle(){}},addEventListener(){},querySelector(){return null;},getAttribute(){return null;},showModal(){this.open=true;},close(){this.open=false;}});return nodes.get(selector);}
...
let revision=2,conflictNextEdit=false,polled=false;const posts=[];
...
const sandbox={document:{querySelector:node,querySelectorAll(){return[];}},fetch:async(url,options={})=>{if(options.method==='POST'){const body=JSON.parse(options.body);posts.push(body);if(body.action==='edit'&&conflictNextEdit){conflictNextEdit=false;return {ok:false,json:async()=>({error:'Scene conflict: another editor saved this scene.'})};}if(body.action!=='cancel')revision++;return {ok:true,json:async()=>scene()};}return {ok:true,json:async()=>url==='/api/av'?{projects:[{id:'scene-a',name:'Opening scene',revision,status:polled?'completed':'saved',render_stale:!polled}],capabilities:fixture.capabilities}:scene()};},location:{search:'',assign(){}},URLSearchParams,URL,CSS:{escape:value=>value},setInterval(){return 1;},clearInterval(){},console};
...
setImmediate(()=>{(async()=>{
  ...
  vm.runInContext("avDocument.render.status='completed';setDraft('shots','shot-a','frames','60');renderDocument()",sandbox);html=node('#avContent').innerHTML;
  assert.match(html,/1 clip change is unsaved/);assert.match(html,/Preview and document actions use saved revision 2/);assert.match(html,/Discard unsaved changes/);
  ...
  await click({'[data-discard-drafts]':{dataset:{}}});assert.equal(vm.runInContext('avDrafts.size',sandbox),0);assert.match(node('#avStatus').textContent,/Discarded unsaved clip changes/);assert.doesNotMatch(node('#avContent').innerHTML,/clip change is unsaved/);
  ...
  console.log('Scene editor frontend preserves drafts, blocks document actions, advances saved revisions and refreshes render status.');
