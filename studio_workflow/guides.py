...
def guides():
    ...
    items = [
        ...
        dict(id='workflow', title='Build my own node workflow', summary='Installed nodes → typed connections → checked API export.', steps=[
            ...
            step('Save and hand off', 'Save the Studio document for further editing or export the checked API graph. This first slice does not submit arbitrary edited graphs. Native custom widgets and visual/subgraph import remain explicit adapter work.', '/workflow-studio.html#builder', '#exportGraph'),
        ]),
        dict(id='agents', title='Run a recipe without a browser', summary='Discover → prepare a pinned ticket → approve once → observe.', steps=[
            ...
            step('Discover and prepare', 'Use python -m studio_workflow capabilities, catalog and prepare --recipe recipe.json --out ticket.json. Preparation pins a registered recipe and checks its current bindings. It may stage local reference copies, but submits no model job.', '/workflow-studio.html#agents', '#agentCommands'),
            step('Approve the exact ticket', 'Use run --ticket ticket.json --approve. The same retained ticket maps to the same job, including uncertain jobs. A new ticket is a new intentional attempt; do not create one to work around an unknown outcome.', '/workflow-studio.html#agents', '#agentCommands'),
    ...
    return {'version': 1, 'guides': items, 'generation_submitted': False,
