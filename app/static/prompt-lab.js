...
function saveDraft(){try{const s=globalThis.localStorage;if(!s||typeof s.setItem!=='function')return;const raw=JSON.stringify(draftPayload());if(raw.length>DRAFT_MAX)return;s.setItem(DRAFT_KEY,raw);}catch(e){}}
function loadDraft(){try{const s=globalThis.localStorage;if(!s||typeof s.getItem!=='function')return false;const raw=s.getItem(DRAFT_KEY);if(!raw)return false;let data;try{data=JSON.parse(raw);}catch(e){s.removeItem(DRAFT_KEY);return false;}if(!data||data.version!==DRAFT_VERSION||!data.intent||typeof data.intent.brief!=='string'||typeof data.intent.task!=='string')throw Error('bad draft');intent={schema_version:1,id:'creative-brief',task:data.intent.task,brief:data.intent.brief,facets:data.intent.facets||{},tags:data.intent.tags||[],avoid:data.intent.avoid||[],constraints:data.intent.constraints||[],references:(data.intent.references||[]).map(r=>({id:r.id,role:r.role,kind:r.kind,path:r.path,sha256:r.sha256,take:r.take||[],ignore:r.ignore||[],label:r.label})),verbatim:data.intent.verbatim||{},parameters:data.intent.parameters||{},locked:data.intent.locked||['verbatim']};if(typeof data.profile==='string'&&registry.some(p=>p.id===data.profile))$('profile').value=data.profile;show();summary();words();return true;}catch(e){try{globalThis.localStorage.removeItem(DRAFT_KEY);}catch(_){}return false;}}
