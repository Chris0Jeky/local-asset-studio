...
function invalidate(){resetBuild();queueDraft();schedule();}
const DRAFT_KEY='studio.promptLab.draft',DRAFT_VERSION=1,DRAFT_MAX=65536;let draftTimer=null;
function draftPayload(){collect();const refs=(intent.references||[]).map(r=>({id:r.id,role:r.role,kind:r.kind,path:r.path,sha256:r.sha256,take:r.take||[],ignore:r.ignore||[],label:r.label}));return {version:DRAFT_VERSION,profile:$('profile').value,intent:{schema_version:1,id:'creative-brief',task:intent.task,brief:intent.brief,facets:intent.facets,tags:intent.tags,avoid:intent.avoid,constraints:intent.constraints,references:refs,verbatim:intent.verbatim,parameters:intent.parameters,locked:intent.locked}};}
function saveDraft(){const s=globalThis.localStorage;if(!s||typeof s.setItem!=='function')return;try{const raw=JSON.stringify(draftPayload());if(raw.length>DRAFT_MAX)throw Error('draft too large');s.setItem(DRAFT_KEY,raw);}catch(e){try{s.removeItem(DRAFT_KEY);}catch(_){}}}
function loadDraft(){const s=globalThis.localStorage;if(!s||typeof s.getItem!=='function')return false;const drop=()=>{try{s.removeItem(DRAFT_KEY);}catch(_){}return false;};let data;try{data=JSON.parse(s.getItem(DRAFT_KEY)||'null');}catch(e){return drop();}if(!data)return false;const d=data.intent,obj=v=>v===undefined||(v!==null&&typeof v==='object'&&!Array.isArray(v)),arr=v=>v===undefined||Array.isArray(v)&&v.every(x=>typeof x==='string'),refs=d&&(d.references===undefined||Array.isArray(d.references)&&d.references.every(r=>r&&typeof r==='object'&&typeof r.id==='string'&&arr(r.take)&&arr(r.ignore)));if(data.version!==DRAFT_VERSION||!d||typeof d.brief!=='string'||typeof d.task!=='string'||!obj(d.facets)||!obj(d.verbatim)||!obj(d.parameters)||!arr(d.tags)||!arr(d.avoid)||!arr(d.locked)||!Array.isArray(d.constraints||[])||!refs)return drop();const profile=registry.find(p=>p.id===data.profile&&p.tasks.includes(d.task))||registry.find(p=>p.tasks.includes(d.task));if(!profile)return drop();const previous=intent;try{intent={schema_version:1,id:'creative-brief',task:d.task,brief:d.brief,facets:d.facets||{},tags:d.tags||[],avoid:d.avoid||[],constraints:d.constraints||[],references:(d.references||[]).map(r=>({id:r.id,role:r.role||'identity',kind:r.kind,path:r.path,sha256:r.sha256,take:r.take||[],ignore:r.ignore||[],label:r.label})),verbatim:d.verbatim||{},parameters:d.parameters||{},locked:d.locked||['verbatim']};$('profile').value=profile.id;show();summary();words();$('draft-restored').hidden=false;return true;}catch(e){intent=previous;try{show();}catch(_){}return drop();}}
function startFresh(){clearTimeout(draftTimer);draftTimer=null;try{globalThis.localStorage?.removeItem(DRAFT_KEY);}catch(_){}const p=registry.find(x=>x.id===$('profile').value);intent={schema_version:1,id:'creative-brief',task:p?.tasks?.[0]||'image',brief:$('brief').defaultValue||'',facets:{},tags:[],avoid:[],constraints:[],references:[],verbatim:{},parameters:{},locked:['verbatim']};$('draft-restored').hidden=true;show();summary();words();resetBuild();schedule();}
function queueDraft(){if(typeof setTimeout!=='function')return;clearTimeout(draftTimer);draftTimer=setTimeout(()=>{draftTimer=null;saveDraft();},400);}
// An explicit repair rebuilds once and keeps its own explanation; no debounced rebuild follows to wipe it.
function rebuild(note){resetBuild();queueDraft();build(note);}
