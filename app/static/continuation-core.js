...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.StudioContinuation=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function destinations(intent,presets,source){
    ...
    const prefer={edit:['qwen-1ref','krea-refine'],repair:['anime-detail-fix','krea-refine','anime-esrgan-2x'],restyle:['restyle-wai','restyle-nova','style-pose-wai','style-pose-nova','style-pose-yumeflux'],animate:['wan22-i2v'],mesh:['trellis-auto-cutout']}[intent]||[];
  ...
  function guidance(preset,source){
    ...
    const text=[{'image-to-image':'Resamples the attached image, rather than starting with an empty image. Identity, style and background can still drift.','localized-detail':'Detects and repaints local regions. Detection can miss a face or hand; inspect the output before accepting it.','instruction-edit':'Uses the source as visual context. Write the change you want and what should stay the same.','upscale':'Enlarges the source without a text prompt. This does not repair pose or guarantee identical fine detail.','masked-repair':'Needs a prepared RGBA PNG: transparent alpha identifies the repair region. A plain source copy is not enough.','restyle':cap.keeps_picture?'Keeps this picture (layout, pose, costume, colours; Denoise says how much may change) and repaints it in the look of the pictures you put on the style board. The prompt says who the character is.':'Keeps this picture’s pose and paints a new image in the look of the pictures you put on the style board. Its colours, costume and background are not copied; the prompt says who the character is.','image-to-video':'Uses the source as a visual input. Describe motion, timing and camera movement; an image caption alone is not a motion brief.','image-to-3d':'Uses the source for reconstruction. Hidden surfaces are inferred; inspect the mesh and materials.'}[cap.operation]||cap.scope];
    ...
    if(cap.operation==='restyle')text.push('Your picture becomes the '+(preset.last_reference_label||'pose picture').toLowerCase()+'. After preparing, add one to three pictures whose look you want to the style board.'+(cap.keeps_picture?' Eyes and lashes get a face pass with the same styled model.':' To keep the costume and background as well, choose a Restyle a picture recipe.'));
