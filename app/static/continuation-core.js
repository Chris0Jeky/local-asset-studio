...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.StudioContinuation=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function destinations(intent,presets,source){
    ...
    const prefer={edit:['flux-edit','qwen-1ref','krea-refine'],repair:['anime-detail-fix','krea-refine','anime-esrgan-2x'],restyle:['restyle-klein','restyle-klein-picture','restyle-wai','style-pose-wai','style-pose-nova','style-pose-yumeflux'],combine:['combine-klein'],animate:['wan22-i2v'],mesh:['trellis-auto-cutout']}[intent]||[];
  ...
  function blockerItems(claim,preset,controls,parents,refs=[]){
    ...
    if(sourceInput(cap)==='last_reference'){
      ...
      else if(pictures<minimum)add('board',cap.operation==='combine'?'Add the picture whose pose you want to Picture 1.':'Add at least '+minimum+' picture'+(minimum===1?'':'s')+' whose look you want to the style board (Picture 1).');
  ...
  function guidance(preset,source){
    ...
    if(cap.operation==='combine')text.push('Your picture goes on '+sourceLabel(preset)+' and stays who it is: face, hair, hat, clothing, colours and rendering style. After preparing, add the picture whose pose you want to Picture 1 (image 2). The wording is prepared for you; a third picture leaks its costume unless you say exactly what to take from image 3.');
