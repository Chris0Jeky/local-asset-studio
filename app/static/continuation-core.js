...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.StudioContinuation=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function destinations(intent,presets,source){
    ...
    const prefer={edit:['flux-edit','qwen-1ref','krea-refine'],repair:['anime-detail-fix','krea-refine','anime-esrgan-2x'],restyle:['restyle-klein','restyle-klein-picture','restyle-wai','style-pose-wai','style-pose-nova','style-pose-yumeflux'],combine:['combine-klein-9b-copypose','combine-klein-9b-skeleton','combine-klein-9b-depth','combine-klein-9b-replace','combine-klein-9b','combine-klein'],animate:['wan22-i2v'],mesh:['trellis-auto-cutout']}[intent]||[];
  ...
  function guidance(preset,source){
    ...
    else if(cap.operation==='restyle'&&preset.reference_board&&cap.board_min===0)text.push('Your picture goes on '+sourceLabel(preset)+'. The style board is optional. Leave it empty to use the recipe’s wording and settings, or attach pictures whose look you want to borrow. Your source picture is still required.');
    else if(cap.operation==='restyle'&&!preset.reference_board)text.push('Your picture goes on '+sourceLabel(preset)+'. The wording is prepared for you: the finish to draw, what to keep, and this output’s submitted description when one exists. There is no style board to fill.');
