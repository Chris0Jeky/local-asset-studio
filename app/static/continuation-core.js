...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.StudioContinuation=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function destinations(intent,presets,source){
    ...
    // Combine's declared lead follows a strong pose and must outrank a 4B source-family match; hard runtime/mask blockers still win.
    const score=p=>(p.runtime_block?1000:0)+(p.continuation_capability.requires_mask?500:0)+(intent==='restyle'&&p.continuation_capability.keeps_picture?-300:0)+(intent==='combine'&&rank(p)===0?-200:0)+(family&&p.family===family?-100:0)+rank(p);
