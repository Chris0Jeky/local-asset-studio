...
let avProjects=[],avId=null,avDocument=null,avDrafts=new Map(),avSelection=null,avPolling=null,avPollBusy=false,avWorkspace=[];
...
function sameOriginUrl(value){return typeof value==='string'&&value.startsWith('/')?value:'';}
...
function setDraft(section,id,field,value){draftFor(section,id)[field]=value;}
function clearDraft(section,id){avDrafts.delete(section+':'+id);}
...
async function loadScene(id=avId,poll=false){
  ...
  if(id!==avId)return;avDocument=document;renderDocument();maintainPolling();
...
function renderClip(section,clip){
  ...
  let inputs='';
  if(isShot)inputs=clipInput(section,clip,'source_in','Source in','integer','frames · '+seconds(draftValue(section,clip,'source_in'),rate),0,doc.timing?.frames??'')+clipInput(section,clip,'frames','Length','integer','frames · '+seconds(draftValue(section,clip,'frames'),rate),1,doc.timing?.frames??'')+clipInput(section,clip,'transition_frames','Dissolve','integer','frames · '+seconds(draftValue(section,clip,'transition_frames'),rate),0,doc.timing?.frames??'');
...
function renderTimeline(){
  ...
  const choice=avSelection?.section==='audio'?'samples':'frames';return '<section class="panel av-timeline"><div class="section-title"><div><h3>Timeline</h3><small>'+doc.timing.frames+' frames · '+seconds(doc.timing.frames,rate)+' · '+doc.timing.samples+' samples</small></div></div><div class="av-timeline-track">'+blocks+'</div><div class="av-timeline-controls"><button data-move="-1" '+(selectedIndex<=0?'disabled':'')+'>Move earlier</button><button data-move="1" '+(selectedIndex<0||selectedIndex>=shots.length-1?'disabled':'')+'>Move later</button><label>Split clip<select id="splitClip">'+splitChoices()+'</select></label><label>At <input id="splitAt" type="number" min="1" step="1" value="1"><small id="splitUnit">'+choice+'</small></label><button data-split>Split</button></div></section>';
}
...
function renderHistory(){return '<details class="av-history"><summary>Revision history · '+(avDocument.history||[]).length+'</summary><div class="av-history-list">'+(avDocument.history||[]).slice().reverse().map(item=>'<div class="av-history-item"><b>r'+esc(item.revision)+' · '+esc(item.action)+'</b><span>'+esc(item.summary||'')+' · '+esc(item.actor||'')+' · '+esc(item.at||'')+'</span>'+(item.revision!==avDocument.revision?'<button data-restore="'+esc(item.revision)+'">Restore as new revision</button>':'')+'</div>').join('')+'</div></details>';}
function renderDocument(){
  ...
  $('#avContent').innerHTML='<section class="panel"><div class="av-document-head"><div><span class="eyebrow">SCENE</span><h2>'+esc(doc.name)+'</h2><p class="av-revision">Revision '+doc.revision+' · '+esc(doc.status||'saved')+'</p></div><div class="av-document-actions"><button id="documentReload">Reload server revision</button></div></div>'+(conflicts?'<p class="av-warning">The server changed this scene. Your unsaved fields are still retained here. Reload to compare, then save the fields you want to keep.</p>':'')+'</section>'+renderPreview(doc)+renderTimeline()+'<section class="av-sections">'+renderSection('shots','Shots · cuts and dissolves')+renderSection('audio','Audio · trim, gain and fades')+renderSection('overlays','Static overlays')+'</section>'+renderSources()+renderHistory();
}
function clipBy(section,id){return (avDocument?.project?.[section]||[]).find(clip=>clip.id===id);}
function collectChanges(section,id){const clip=clipBy(section,id),fields=[...document.querySelectorAll('[data-section="'+CSS.escape(section)+'"][data-clip="'+CSS.escape(id)+'"]')];if(!clip||!fields.length)throw Error('That clip is no longer available. Reload the scene.');const changes={};for(const input of fields){const field=input.dataset.field,value=input.type==='checkbox'?input.checked:input.value;changes[field]=input.type==='checkbox'?value:(field==='bus'?value:(field==='gain_db'||field==='opacity'?number(value,field):integer(value,field)));}return changes;}
async function mutate(body,success){try{const doc=await avPost('/api/av/'+encodeURIComponent(avId),{...body,expected_revision:avDocument.revision,actor:'user'});avDocument=doc;if(success)success();renderDocument();maintainPolling();await refreshProjects(avId);avMessage('Saved revision '+doc.revision+'.');}catch(error){if(/^Scene conflict:/i.test(error.message)){if(avSelection)draftFor(avSelection.section,avSelection.id).conflict=true;renderDocument();}avMessage(error.message,true);}}
...
$('#avProjectList').onclick=event=>{const project=event.target.closest('[data-project]');if(!project)return;stopPolling();avId=project.dataset.project;avDrafts.clear();avSelection=null;renderProjects();loadScene(avId).catch(error=>avMessage(error.message,true));};
