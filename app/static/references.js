...
function renderReferenceSlots(){
  ...
  $('#referenceSummary').textContent=referencePending?'Uploading and validating…':selected.reference_board?filled+' / '+referenceRecords.length+' style pictures on the board · the adapter blends them; empty slots are skipped · output size follows width and height':filled+' / '+referenceRecords.length+' attached · each scaled to 1.0 MP; output size follows width and height';
  updateReady();
}
