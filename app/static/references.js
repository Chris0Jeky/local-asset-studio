...
function renderReferenceSlots(){
  ...
  $('#referenceCards').innerHTML=referenceRecords.map((r,i)=>'<article class="reference-card" data-ref-drop="'+i+'"><div class="section-title"><b>Image '+(i+1)+'</b><div><button type="button" data-ref-up="'+i+'" '+(!i?'disabled':'')+' aria-label="Move Image '+(i+1)+' earlier">↑</button><button type="button" data-ref-down="'+i+'" '+(i===referenceRecords.length-1?'disabled':'')+' aria-label="Move Image '+(i+1)+' later">↓</button><button type="button" data-ref-clear="'+i+'">Clear</button></div></div><label>Role for Image '+(i+1)+'<select data-ref-role="'+i+'">'+referenceRoles.map(role=>'<option '+(r.role===role?'selected':'')+'>'+role+'</option>').join('')+'</select></label><label class="reference-drop">'+(r.file&&!r.missing?'<img src="/api/uploads/'+encodeURIComponent(r.file)+'" alt="Image '+(i+1)+' reference"><span>'+r.width+' × '+r.height+'</span>':'<span>'+(r.missing?'Reference missing. Attach it again.':'Drop or choose an image')+'</span>')+'<input type="file" data-ref-file="'+i+'" accept="image/png,image/jpeg,image/webp" aria-label="Upload Image '+(i+1)+'"></label><label>Use from Image '+(i+1)+'<textarea rows="2" data-ref-contribution="'+i+'">'+esc(r.contribution)+'</textarea></label><label>Avoid copying<textarea rows="2" data-ref-avoid="'+i+'">'+esc(r.avoid)+'</textarea></label></article>').join('');
  $('#referenceSummary').textContent=referencePending?'Uploading and validating…':referenceRecords.filter(r=>r.file&&!r.missing).length+' / '+referenceRecords.length+' attached · output aspect follows Image 1';
  updateReady();
}
