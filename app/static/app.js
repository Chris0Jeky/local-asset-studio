...
function applyRecipe(recipe) {
  ...
  const controls = continuationState ? StudioContinuation.settings(selected, recipe.controls, values()) : (recipe.controls || {});
  Object.entries(controls).forEach(([k,v]) => { const el = k === 'positive' ? $('#positive') : k === 'negative' ? $('#negative') : getControl(k); if (el) el.value = v; });
