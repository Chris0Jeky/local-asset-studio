...
(function(){
  ...
  const referenceHeading=element('div','ux-section-heading','<span>02</span><h3>Bring in your sources</h3><button id="uxPullAsset">Pull from library</button>');q('#roleReferences').before(referenceHeading);
  ...
  function syncCreate(){if(!selected)return;q('#uxRecipeLabel').textContent=selected.name;referenceHeading.hidden=!selected.reference&&!selected.reference_slots?.length;q('#uxSourceNote').hidden=referenceHeading.hidden;syncReady();}
  ...
  after('openAsset',()=>{q('#uxAssetUnsaved')?.remove();if(!activeAsset)return;const a=activeAsset;const eligibility=U.sceneEligibility([a]);q('#assetHandoffs').innerHTML=(!a.trashed_at&&a.media_type==='image'?'<button class="primary" data-ux-handoff="'+a.id+'">Continue with this →</button>':'')+(!a.trashed_at&&['image','video','audio'].includes(a.media_type)?'<a class="ux-scene-link" href="/av.html?asset_ids='+encodeURIComponent(a.id)+'">'+(eligibility.ok?'Use in a scene':'Open scene source picker')+' ↗</a>':'');q('#assetRecipe').disabled=!a.job_id;});
  ...
  q('#uxPullAsset').onclick=async()=>{if(!selected?.reference)return;const epoch=selectionEpoch;await refreshAssets();if(epoch!==selectionEpoch||view!=='create')return;q('#uxSourceSearch').value='';q('#uxPickerStatus').textContent='Attach a copy. The original stays in your library.';q('#uxSourceSlot').innerHTML=selected.reference_slots?.length?selected.reference_slots.map((r,i)=>'<option value="'+i+'">Picture '+(i+1)+' · '+escape(referenceRecords[i]?.role||r.role)+'</option>').join(''):'<option value="reference">Reference / first frame</option>'+(selected.last_reference?'<option value="lastReference">Last frame</option>':'');renderSources();picker.showModal();};
  q('#uxSourceSearch').oninput=renderSources;
  q('#uxSourceAssets').onclick=async e=>{const button=e.target.closest('[data-ux-pull]');if(!button||pickerBusy)return;pickerBusy=true;button.disabled=true;syncReady();try{const id=button.dataset.uxPull,slot=q('#uxSourceSlot').value;if(continuationState&&(slot==='reference'||slot==='0'))throw Error('Use Change route or Continue with the new asset to replace this task’s source.');const stamp=workbenchStamp(),result=await post('/api/assets/reference',{id});if(stamp!==workbenchStamp())throw Error('The workbench changed during attachment. Reopen the picker.');let previous=null;if(selected.reference_slots?.length){previous=referenceRecords[Number(slot)].parent_asset;Object.assign(referenceRecords[Number(slot)],result,{missing:false});if(Number(slot)===0)uploaded=result.file;renderReferenceSlots();}else if(slot==='lastReference'){lastUploaded=result.file;q('#lastReference').value='';pendingInputs.delete('lastReference');}else{uploaded=result.file;q('#reference').value='';pendingInputs.delete('reference');}replaceParentAsset(slot==='lastReference'?'lastReference':'reference',previous,id);picker.close();draftDirty=true;saveDraft();syncCreate();announce('Saved source attached. No generation submitted.');}catch(err){q('#uxPickerStatus').textContent=err.message;}finally{pickerBusy=false;button.disabled=false;syncReady();}};
