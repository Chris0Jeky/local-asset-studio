...
The adapter supplies absolute owned paths, `QT_QPA_PLATFORM=offscreen`, no shell,
no window-creating console, a fixed 90-second timeout, and no scripts, macros or
caller command text. The CLI intentionally uses only its fixed default configured
path; a trusted coordinator may call `execute(..., configured_krita=...)` after
it has validated its own configuration.
...
September 2026. One actual 1×1 synthetic ORA attempt was made with this adapter;
the first offscreen batch export exited `0xC0000005` before creating a KRA and
without stderr. Its source snapshot, argv log and failure receipt remain in the
ignored worktree runtime evidence. Therefore this slice did **not** prove an
actual native KRA save/reopen or PNG export. No installer was run. A successful
local `roundtrip` result is the condition that sets
`native_kra_save_reopen_proved` to true for that one output directory.
