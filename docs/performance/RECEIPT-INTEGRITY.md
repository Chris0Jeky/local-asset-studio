...
`incomplete=True` (the capture is not a finished four-artifact set; it may still be writing,
or it may have failed before the full set was written). A complete four-name manifest whose
listed sidecar is gone is `artifact_missing` with `incomplete=False` (`invalid`): a broken
finished claim, not an in-progress capture. CLI `integrity: incomplete` is not a reason to
wait on a deleted listed file. No exception message includes receipt content or the nominated path.
