...
- no planning or prompt-projection record grants runtime authority;
...
Run the standard-library-only offline gates before changing a programme manifest or deterministic prompt profile:
...
The foundation validator checks authority, adult/content declarations, route evidence states, control and benchmark references, zero authorized candidate caps, adapter sweep bounds and duplicate IDs. The intelligence validator checks prompt dialect isolation, vocabulary provenance and collisions, technique availability, provider-specific source identity, immutable revisions and hash-pinned file selection. The prompt compiler adds deterministic route-specific text while keeping non-prompt controls unresolved. The source-fetch and acquisition-plan help commands exercise argument registration only and perform no provider access. None of these validation commands downloads bytes, installs a component, submits generation or grants runtime authority.
...
2. Review the #437 deterministic prompt-profile and taxonomy slices, then qualify formatting against unchanged/manual prompts under #37/#409; software tests alone do not prove artistic benefit.
