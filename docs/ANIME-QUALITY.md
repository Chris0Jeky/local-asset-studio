...
The repair graph is **LoadImage -> VAEEncode -> SetLatentNoiseMask -> KSampler -> VAEDecode -> ImageCompositeMasked -> SaveImage**. The final composite restores the original where the mask is zero, instead of saving the full VAE reconstruction. Soft-mask boundaries blend source and repair. Studio requires a real RGBA PNG with a nonempty transparent repair region and dimensions divisible by 8; it refuses unaligned sources rather than padding or cropping them. Pixel preservation is a graph design property here, not a newly executed image-comparison test.
