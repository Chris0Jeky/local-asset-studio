...
| `POST /api/gpu-lease` | `{"holder": "local-qwen", "ttl_seconds": 7200}` | `200` with the state plus `granted: true`, `renewed` and `stopped_now`. The same holder calling again renews the lease with a new TTL counted from now. |
