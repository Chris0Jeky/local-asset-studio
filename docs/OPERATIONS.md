...
- A grant needs the same checks a backend switch makes before it stops anything. No backend switch can be running (`409 backend_switch_active`). No Studio job can be queued, submitting, running or uncertain, and no experiment can be active (`409 studio_work_active`). Every managed endpoint must report an empty `queue_running` and `queue_pending`, with every listener and startup process verified as a Studio-configured launch (`409 backend_not_idle`: the reason names a busy queue, an unknown queue state, or a foreign or ambiguous listener). Nothing is stopped when a grant is refused. The only processes stopped are verified Studio-owned listeners, and each one's queue is checked again just before it is terminated. A process that does not exit within 15 s gives `500 stop_unconfirmed` and no lease; check `stopped_processes` in the body before retrying.
...
- Not covered: a ComfyUI that someone starts by hand during the lease keeps running until the holder renews (renewal stops it again if it is idle). Work the Studio runs outside ComfyUI (native Krita, Blender or Godot jobs, helper probes) is not paused.
