...
The launcher reuses the installed AMD ComfyUI runtime and weights. It checks whether ComfyUI is running and consults the retained Studio GPU lease before starting an offline backend. A held lease skips ComfyUI but still starts the studio for lease status/release; unreadable or malformed lease state stops the launcher for inspection. Without a live lease it starts ComfyUI if needed, starts the studio, and opens the page. Running it again should open the same service. The studio serves only `127.0.0.1:8191`; it is not a network deployment.
...
The studio submits jobs serially and waits for the ComfyUI queue. Avoid simultaneously pressing Generate in both interfaces. A user can still independently submit work in ComfyUI; the studio is not a global GPU lock. The GPU lease below only pauses Studio-owned ComfyUI for one other local GPU tenant.

### GPU lease: lending the GPU to another local tenant (#805)

An idle ComfyUI keeps about 2-2.6 GiB of dedicated VRAM, enough to push the local Qwen model (local-llm-ops, LM Studio) into WDDM shared memory. Stopping ComfyUI by hand does not hold: with `runtime_auto_recover: true`, runtime recovery starts it again on its next pass, about 40 s later (measured by local-llm-ops on 24 September 2026, its docs/LOADER-STATE.md). The GPU lease is how another local tenant asks the Studio to stop its own idle ComfyUI and keep it stopped for a bounded time.

| Route | Body | Result |
| --- | --- | --- |
| `GET /api/gpu-lease` | none | `200` with `held`, `holder`, `acquired_at`, `renewed_at`, `expires_at` (epoch seconds), `expires_at_utc`, `ttl_seconds`, `remaining_seconds`, `stopped_processes`, `last_release` (`released` or `expired`), `limits` and `relaunch_on_release` (`automatic` when runtime recovery is enabled, otherwise `manual`). `GET /api/backends` carries the same object as `gpu_lease`. |
| `POST /api/gpu-lease` | `{"holder": "local-qwen", "ttl_seconds": 7200}` | `200` with the state plus `granted: true`, `renewed`, `stopped_now` and `ignored_stale_uncertain` (the job ids the stale-uncertain rule below let through; always `[]` on a renewal). The same holder calling again renews the lease with a new TTL counted from now. |
| `POST /api/gpu-lease/release` | `{"holder": "local-qwen"}` | `200` with the state plus `released` (`false` when nothing was held; release is idempotent). |

Rules:

- `holder` is 1-64 characters of `a-z`, `0-9`, `.`, `_`, `-`. `ttl_seconds` is mandatory and must be a whole number from 60 to 14400 (4 hours), so a crashed holder cannot keep ComfyUI down for longer than that. Unknown fields are refused. Any of these gives `400 invalid_lease_request`.
- A grant needs the same checks a backend switch makes before it stops anything. No backend switch can be running (`409 backend_switch_active`). No Studio job can be queued, submitting, running or uncertain, and no experiment can be active (`409 studio_work_active`; its body also carries `ignored_stale_uncertain`). Every managed endpoint must report an empty `queue_running` and `queue_pending`, with every listener and startup process verified as a Studio-configured launch (`409 backend_not_idle`: the reason names a busy queue, an unknown queue state, or a foreign or ambiguous listener). A preflight refusal stops nothing. Strict queue and process-identity checks finish for every selected listener before the first stop; immediate per-stop rechecks remain. A later external queue change can still refuse after an earlier idle listener was stopped, so always inspect `stopped_processes`. Only verified Studio-owned listeners are stopped. A process that does not exit within 15 s gives `500 stop_unconfirmed` and no lease; check `stopped_processes` in the body before retrying.
- Stale uncertain jobs age out for the lease only (owner decision 25 September 2026, #979). A job still in `uncertain` whose latest recorded activity is at least 24 hours old (`STALE_UNCERTAIN_SECONDS` in `app/gpu_lease.py`, also reported as `limits.stale_uncertain_seconds`) no longer blocks a grant. Activity means the latest finite `created_at`, `started_at`, `finished_at`, pending-submission `marked_at`, or tracking stop/resume `recorded_at`; a put-away acknowledgement does not count. A job with no readable time, or a time in the future, still blocks. A younger uncertain job still blocks, and queued, waiting, submitting and running jobs block at any age. The grant does not change the job: it is not resubmitted, resolved, abandoned or stopped, and it stays resumable. Its prompt IDs still get the in-memory history check: if a running ComfyUI still holds a result for one, the grant refuses with `409 backend_not_idle` and nothing is stopped; resume its observation first. That check needs a known prompt ID: a stale uncertain job whose submission never returned one (`pending_submission` with no `prompt_ids`) has nothing to look up, so a grant can stop a ComfyUI whose in-memory history still holds that result; its recipe and workflow stay in `experiments/runs/<job-id>/`. The granted response and the `granted` line in `.runtime/studio-gpu-lease.log` list the ignored ids in `ignored_stale_uncertain`. Backend switches, job admission and experiments are unchanged: an uncertain job of any age still blocks a switch.
- Only one holder at a time: another holder gets `409 lease_held`, with `holder` and `expires_at`. A release names the holder, and a different name also gets `409 lease_held`.
- While the lease is held, runtime recovery reports `leased` and starts nothing. `POST /api/jobs` and experiment start/resume return `409 gpu_leased`, naming the holder and the expiry, and create nothing. A backend switch returns `409 gpu_leased`. New registered workflow/MCP ticket dispatch is refused before durable dispatch intent or job creation. Both `POST /api/workflow-studio/run` and `POST /api/workflow-studio/document-runs/<request_id>/run` return `409 gpu_leased` with `dispatch_attempted: false`, holder and expiry; there is no new job to find. Keep the original ticket or saved run and its approval hashes, then explicitly retry that same identity after release or expiry. Existing job receipts and orphaned intents remain recoverable during a lease without a second dispatch. A different caller's job that reaches the worker despite the admission guards can still end `not_submitted`; that worker fallback is not the result of a pre-intent ticket refusal.
- On release or expiry the Studio does not launch anything itself. With `runtime_auto_recover: true`, recovery starts the selected backend with its configured arguments on its next pass (poll interval plus startup, about 40 s). Otherwise start it with `Start Studio.cmd` or an explicit backend switch.
- The lease is saved in `.runtime/studio-gpu-lease.json` and its grants, renewals, releases and expiries are appended to `.runtime/studio-gpu-lease.log`. It survives a Studio restart until it expires, so a restarted monitor cannot relaunch ComfyUI under a live holder.
- There is no authentication. The Studio binds only `127.0.0.1:8191`, requires a loopback `Host` (`127.0.0.1:8191` or `localhost:8191`), and requires `Origin: http://127.0.0.1:8191` (or `localhost`) on every POST. A non-browser client sets that header explicitly.
- The desktop launcher reads `.runtime/studio-gpu-lease.json` before either startup and rechecks immediately before invoking the external ComfyUI launcher. It never creates, renews, releases, clamps, or repairs a lease. A malformed, oversized, linked, or unreadable record fails closed; inspect the retained file rather than deleting it to bypass the guard. A valid held lease leaves Studio startup available for status/release.
- Not covered: a ComfyUI started by hand or by another launcher during a lease is not stopped by renewal. Same-holder renewal only updates the deadline. The desktop guard is a read-only observation, not a cross-process reservation: another process can change lease state after the final check. Work outside ComfyUI (native Krita, Blender or Godot jobs, helper probes) is not paused. See [GPU lease failure semantics](GPU-LEASE-FAILURE-SEMANTICS.md).

From Windows PowerShell 5.1, which throws on a 409 (the JSON reason is in the error details):

```powershell
$studio = 'http://127.0.0.1:8191'; $origin = @{ Origin = $studio }
Invoke-RestMethod -Method Post -Uri "$studio/api/gpu-lease" -Headers $origin -ContentType 'application/json' -Body '{"holder":"local-qwen","ttl_seconds":7200}'
Invoke-RestMethod -Uri "$studio/api/gpu-lease"
Invoke-RestMethod -Method Post -Uri "$studio/api/gpu-lease/release" -Headers $origin -ContentType 'application/json' -Body '{"holder":"local-qwen"}'
```
