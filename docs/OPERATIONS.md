...
- The lease is saved in `.runtime/gpu-lease.json` and its grants, renewals, releases and expiries are appended to `.runtime/gpu-lease.log`. It survives a Studio restart until it expires, so a restarted monitor cannot relaunch ComfyUI under a live holder.
