...
| `animagine40-opt-tags-v1` | Approved tags in their supplied order; optional quality tail must be final | `anime`, `anime-animagine-quality`, `animagine-face` |
...
The same check runs through the existing CLI and composed HTTP route. Only the original registered
...
Tests retain all nine old profile digests, verify all seven actual graph contracts, preserve inputs and
