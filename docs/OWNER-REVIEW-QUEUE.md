...
| 16 | 25 Sep 2026 | Large-job preparation (#306) | Ran your approved proof with two temporary, clearly labelled synthetic admission profiles and both flags on, then restored `config/local.json` byte-for-byte (flags absent, no profile) | `docs/verification/2026-09-25-large-job-preparation-live-proof.md` | No measured profile exists yet (#178), so the proof needed a synthetic one to reach `/free` and the restart | done | |
| 17 | 25 Sep 2026 | Asset library | Marked 1,178 unreviewed, unfavourited, unlabelled lab outputs as "Agent lab", as you approved. The id list is in `.runtime/lab-backlog-mark-0925.json` for a full undo | Asset library → Agent runs | Your library now opens on your own 9 assets | done | |
