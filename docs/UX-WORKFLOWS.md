...
| Asset library · review queue | Queue over unreviewed assets in the current view, keyboard decisions, reason chips, bulk marking, grouping | Every decision is the existing single-asset metadata save with its revision guard; only a confirmed save advances. Grouping and the queue are local projections — the four review states and the server schema are unchanged, and a review is still not artistic or licence approval. |
| Runs & review | Existing planned comparisons, reservations, status, review and export | Plan comparison and Start remain separate. Uncertain work is inspected rather than repeated. Existing review desk keeps its blind-review behavior. An orientation block at the top of the view states the purpose, the three routes in (Plan comparison in Create, Branch this study, export/scene/voice jobs) and one clause per status; it is a `<details>` whose collapse is remembered per browser. Plan names are humanised for reading only - the stored name, its fingerprint and every receipt are untouched, and the full name stays on the list entry as its title. Candidate cards offer Keeper / Needs work, which send the same revision-guarded `/api/assets/update` edit the asset dialog sends and change no comparison outcome. |
| Prompt Lab | Creative intent, profile-specific text compilation | Rebuilds itself as you type; **Build the prompt** stays as an explicit rebuild. Every locked control names its reason next to it, each compiler code renders as a sentence plus an action (the code and raw message stay in small text), and safe repairs — switching to a profile that fits the attached references, keeping avoid terms as a reversible review note — are one click. Explicit **text-only** handoff to Create, which names the matching catalog recipes without selecting one. Compiler fields do not establish executable graph bindings, transfer references, or select a model automatically. |
