...
**Architecture:** A bounded checked-in catalogue defines reviewable profile intent. An optional local registry may overlay a checked-in profile only when it is bound to that exact catalogue record and declares a higher revision. Spoken Brief manifests and receipts bind the resolved profile, delivery, lexicon, mix, and speaker metadata. Execution is admitted before filesystem or network side effects. A separate zero-generation planner and report validator records candidate measurements, line evidence, long-form listening, and owner decisions. Recordings, model files, local paths, and private qualification evidence remain outside Git.
...
- Require exact checked-in catalogue provenance and a higher declared revision for a local profile overlay.
- Do not represent the read-only resolver as a local-file compare-and-swap operation or persisted revision chain; stronger mutation semantics remain #671.
...
- [x] Require a strictly higher revision and exact checked-in `supersedes_profile_sha256` before a local record overlays a catalogue profile.
- [x] Pin the overlay model explicitly: a local successor hash is not treated as a mutable-registry CAS predecessor.
...
- [x] Document checked-in catalogue vs local overlay, exact catalogue-hash binding, selection commands, qualification planning, report validation, and promotion criteria.
- [x] Document that the resolver does not serialize local file writers or retain a predecessor chain; stronger mutation semantics remain #671.
...
8. create a higher local profile overlay bound to the exact checked-in catalogue profile and a genuine producer adapter;
9. if local writers need durable chaining or concurrency control, implement the separate registry update boundary in #671 rather than inferring it from the resolver;
10. keep references, weights, audio, reports, and permission records out of Git.
