    ...
    def stage(self, *, resume_uploads=False):
        with self.locked():
            ...
            for seed in self.handoff['seeds']:
                ...
                require(records[0]['transform']['source_size']==self.handoff['size'], 'Native preprocessing changes candidate dimensions')
                canvas=[preview['workflow'][str(node)]['inputs'][str(field)]
                        for node,field in (self.handoff['native_preset']['width'],self.handoff['native_preset']['height'])]
