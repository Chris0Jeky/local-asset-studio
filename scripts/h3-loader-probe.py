...
install(encoder,read_only=args.read_only);start=time.time()
result={'loader':'stdlib '+('read-only' if args.read_only else 'copy-on-write')+' mmap','encoder':str(encoder),'construct_requested':args.construct,'inference':False}
try:
    if args.construct:
        import comfy.sd
        print('BEGIN bounded CPU encoder construction',flush=True)
        ...
        state=comfy.utils.load_torch_file(str(encoder));result['tensor_count']=len(state)
