...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ('plan', 'run'):
        ...
        command.add_argument('--speaker-id', default='brief-narrator')
        if name == 'run':
...
def main(argv=None) -> int:
    ...
    try:
        if arguments.command == 'plan':
            value = plan(arguments.pack, speaker_id=arguments.speaker_id)
        else:
            value = run(arguments.pack, base_url=arguments.base_url, speaker_id=arguments.speaker_id,
                        poll_seconds=arguments.poll_seconds, deadline_seconds=arguments.deadline_minutes * 60)
