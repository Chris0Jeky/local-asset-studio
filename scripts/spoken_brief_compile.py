...
The command is deliberately a coordinator over the existing Voice baseline API.
It never imports a model, starts Studio, or repeats a terminal/uncertain take.
"""
...
def _clean_inline(text: str, omissions: dict) -> str:
    ...
    def raw_url(match):
        ...
        value = match.group(0); punctuation = ''
        while value and value[-1] in '.,;:!?':
            punctuation = value[-1] + punctuation; value = value[:-1]
...
def compile_markdown(source: str, *, source_name: str = 'brief.md') -> dict:
    ...
    if lines and lines[0].strip() == '---':
        for index in range(1, len(lines)):
            if lines[index].strip() == '---':
                lines = lines[index + 1:]; omissions['front_matter'] = 1; break
    blocks = []
    paragraph = []
    in_code = False
    ...
    for raw in lines:
        ...
        if heading:
            flush(); text = _clean_inline(heading.group(1), omissions)
            if text:
    ...
    for paragraph_index, block in enumerate(blocks):
        ...
        for piece_index, piece in enumerate(pieces):
            segments.append({'id': f'segment-{len(segments) + 1:04d}', 'text': piece,
                             'text_sha256': digest_bytes(piece.encode('utf-8')),
                             'block': paragraph_index, 'kind': block['kind'],
    ...
    return {'schema_version': SCHEMA_VERSION, 'compiler_version': COMPILER_VERSION,
            'source_name': source_name, 'segments': segments, 'omissions': omissions}
...
def compile_source(source: Path, *, speaker_id: str = 'brief-narrator') -> dict:
    ...
    manifest = {'schema_version': SCHEMA_VERSION, 'kind': 'spoken-brief',
                'source': {'path': str(source), 'name': source.name, 'bytes': len(raw), 'sha256': digest_bytes(raw)},
                'speaker_id': speaker_id, 'compiler': {'version': COMPILER_VERSION,
                    'target_line_chars': TARGET_LINE_CHARS, 'max_line_chars': MAX_LINE_CHARS,
                    'max_batch_lines': MAX_BATCH_LINES, 'max_batch_chars': MAX_BATCH_CHARS,
                    'max_batch_words': MAX_BATCH_WORDS, 'max_segments': MAX_SEGMENTS},
                'omissions': compiled['omissions'], 'segments': compiled['segments']}
